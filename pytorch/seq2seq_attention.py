...
class Seq2Seq(nn.Module):
    def __init__(self,enc_v_dim, dec_v_dim, emb_dim, units, max_pred_len, start_token, end_token):
        ...
        self.attn = nn.Linear(units,units)    # 加入attention机制
    ...
    def encode(self,x):
        ...
        o,(h,c) = self.encoder(embedded,hidden) # o->[n, step, units], (h,c)->[num_layers * num_directions, n, units]=[1, 32, 32]
        return o,h,c
    ...
    def train_logit(self,x,y):
        ...
        dec_in = dec_in.type(torch.LongTensor)
        ...
        for i in range(dec_emb_in.shape[0]):
            ...
            # hx.unsqueeze() -> [n, 1, units]
            # o.permute(0,2,1) -> [n, units, step]

            att_weight = softmax(attn_prod, dim=2)  # [n, 1, step] 注意力权重
            context = torch.matmul(att_weight,o)    # [n, 1, units] = [n, 1, step] x [n, step, units] 注意力权重对encoder加权
